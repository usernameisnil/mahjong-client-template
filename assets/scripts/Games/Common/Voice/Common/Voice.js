cc.Class({
    ...
    onLoad: function () {
        ...
        this._voice = cc.find("Canvas/layerRoot/voice");
        this._voice.active = false;
        
        this._voice_failed = cc.find("voice_failed", this._voice);
        this._voice_failed.active = false;
        
        this._timeBar = cc.find("time", this._voice);
        this._timeBar.scaleX = 0.0;
        
        this._volume = cc.find("volume", this._voice);
        for(var i = 1; i < this._volume.children.length; ++i){
            this._volume.children[i].active = false;
        }
        
        var btnfailedOK = cc.find("voice_failed/btn_ok", this._voice);
        if(btnfailedOK){
            cc.vv.utils.addClickEvent(btnfailedOK,this.node,"Voice","onBtnOKClicked");
        }
        
        var self = this;
        var btnVoice = cc.find("Canvas/btnNodes/btn_voice");
        btnVoice.active = cc.vv.replayMgr.isReplay() == false;
        if(btnVoice){
