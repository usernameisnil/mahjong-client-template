cc.Class({
        ...
        this.node.on('game_dipai',function (data) {
            if(cc.vv.PDKReplayMgr.isReplay()){
                ...
                for(let i=0;i<self._paicountLabel.length;++i){
                    if(i===localindex){
                        self._paicountLabel[i].string = 16;
    ...
    showPaiCount_step: function (localindex, holdsNum,holds) {
        if(cc.vv.PDKReplayMgr.isReplay()) {
            ...
            this._paicountLabel[localindex].string = holds.holds.length;
        }else{
    ...
    initOtherHolds: function (index, holds) {
        var hold=this.node.getComponent('PDKGame').sortHolds(holds);
        if (index !== 0) {
