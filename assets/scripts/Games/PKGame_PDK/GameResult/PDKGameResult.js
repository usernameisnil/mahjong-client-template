cc.Class({
    ...
    onLoad: function () {
        ...
        if (btnClose) {
            cc.vv.utils.addClickEvent(btnClose, this.node, "PDKGameResult", "onBtnCloseClicked");
        }
        var btnShare = cc.find("GameEnd6/btnShare", this._gameresult);
        if (btnShare) {
            cc.vv.utils.addClickEvent(btnShare, this.node, "PDKGameResult", "onBtnShareClicked");
        }
        var btnShareFriend= cc.find("GameEnd7/btnFriend", this._gameresult);
        if (btnShareFriend) {
            cc.vv.utils.addClickEvent(btnShareFriend, this.node, "PDKGameResult", "onBtnShareFriend");
        }
        var btnShareFriendCircle= cc.find("GameEnd7/btnFrienCircle", this._gameresult);
        if (btnShareFriendCircle) {
            cc.vv.utils.addClickEvent(btnShareFriendCircle, this.node, "PDKGameResult", "onBtnShareFriendCircle");
        }
    ...
    onGameEnd: function (data) {
        ...
        for (var i = 0; i < cc.vv.gameNetMgr.seats.length; ++i) {
            var zhaCount = this._seats1[i].node.getChildByName('zhaCount');
            var chuntian = this._seats1[i].node.getChildByName('guanMen');
            if(zhaCount && chuntian ){
                zhaCount.active = false;
                chuntian.active = false;
            }

             //显示跑得快单局结算信息
             if(data.pdkEndInfo){
                // var zhaCount = this._seats[i].node.getChildByName("zhaCount");
                // var chuntian = this._seats[i].node.getChildByName("chuntian");
                if(zhaCount && chuntian){
                   if(data.pdkEndInfo[i].zhaCount > 0){
                    zhaCount.active = true;
                    zhaCount.getComponent(cc.Label).string = "炸弹数:" + data.pdkEndInfo[i].zhaCount;
                   }
                    if(data.pdkEndInfo[i].chuntian){
                        chuntian.active = true;
                    }else{
                        chuntian.active = false;
                    }
                }
            ...
            if(!endinfo||(cc.vv.gameNetMgr.numOfGames===cc.vv.gameNetMgr.maxNumOfGames)) {
                ...
                var holds = cc.vv.pdkgame.sortHolds(data.results[i]);
        ...
        // }
        var pkgame = this.node.getComponent('PDKGame');
        if (true === false) {
