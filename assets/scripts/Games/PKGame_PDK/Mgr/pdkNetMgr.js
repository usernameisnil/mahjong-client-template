...
cc.Class({
    ...
    xiaJiaHoldsNum: function(){
        var seats = this.seats
        for(let i = 0; i < seats.length;i ++){
            var chupaiPalyerId = seats[i].userid;
            var seatindex = this.getSeatIndexByID(chupaiPalyerId);
            var localindex = this.getLocalIndex(seatindex);
            if(localindex === 1){
                if(seats[i].holdsNum === 1){  //下家报单
    ...
    doPKChupai: function (seatIndex, pai, holdsNum) {
       ...
       // console.log('2192 meng this.seats', this.seats);
        ...
        if (!cc.vv.PDKReplayMgr.isReplay() && seatData.folds && pai !== null) {
            var tmpfolds = [];
            var pai_len = pai.length;
            for (let i = 0; i < pai_len; i++) {
                tmpfolds.push(pai[i]);
