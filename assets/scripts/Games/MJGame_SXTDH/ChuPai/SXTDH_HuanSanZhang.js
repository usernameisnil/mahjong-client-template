cc.Class({
    ...
    initHuaipaiInfo:function(){
        ...
        for(var i = 0; i < seat.huanpais.length; ++i){
            huaipaiinfo.getChildByName("hp" + (i + 1)).getComponent(cc.Sprite).spriteFrame = cc.vv.controlMgr.getSpriteFrameByMJID("M_",seat.huanpais[i]);
        }
    ...
    onHuanSanZhang:function(event){
        ...
        for(var i = 0; i < this._huanpaiArr.length; ++i){
            ...
            var nt = cc.vv.controlMgr.getMahjongType(pai); 
            if(type == null){
                type = nt;
            }
            else{
