cc.Class({
    ...
    onLoad: function () {
        ...
        this.addComponent("HA_GameOver");
        // this.addComponent("DingQue");
        this.addComponent("HA_PengGangs");
        this.addComponent("MJRoom");
        this.addComponent("TimePointer");
        this.addComponent("HA_GameResult");
        this.addComponent("Chat");
        this.addComponent("Folds");
        this.addComponent("ReplayCtrl");
        this.addComponent("PopupMgr");
        this.addComponent("HuanSanZhang");
        this.addComponent("HA_ReConnect");
        this.addComponent("Voice");
        this.addComponent("UserInfoShow");
        // this.addComponent("run");
       // this.addComponent("HA_hunpaiIcon");
        
        this.otherHoldPos = {};     //其他玩家的位置信息
        this.send_action_tags = {}; //发送动作的标志
        this.huedData = [];         //胡牌玩家的胡的信息
    ...
    start: function () {
        ...
        if (cc.vv.gameNetMgr.gamestate != "" && this._showIPEqual == false) {
            // this.showIPEqual();
        }
        ...
        if (mySeat.ispeng) {
            mySeat.ispeng = false;
            var isAllMask = this.judgeAllMask();
            if (isAllMask == true) {
                this.showHoldClick();
            }
        }
        ...
        // 检测当前是自己轮次，手牌是否有花牌
        if (cc.vv.gameNetMgr.turn != -1 && cc.vv.gameNetMgr.turn == cc.vv.gameNetMgr.seatIndex && mySeat.ismopai == true) {
            mySeat.ismopai = false;
    ...
    syncHunPai: function () {
        ...
        if (cc.vv.gameNetMgr.gamestate == "xiapao" || cc.vv.gameNetMgr.gamestate == "runSelect") {
            return;
        }
    ...
    showTingTriangle: function (data) {
        ...
        if (!cc.vv.replayMgr.isReplay() && conf && conf.type == cc.vv.brightGameName && conf.opType == 4) {
    ...
    initEventHandlers:function(){
        ...
        this.node.on('game_mopai',function(data){
            ...
            if(localIndex == 0){
                ...
                self.showMoPaiMask(sprite.node);
            }
        ...
        this.node.on('buhua_notify',function(data){
            ...
            cc.vv.audioMgr.playMJGameSFX(cc.CGameConfigDataModel.getAudioURLByOther("buhua",data.userid),true);
        });
        ...
        this.node.on('game_huOver',function(data){
            
            var huLength = self.huedData.length;
            if (huLength == 0) {
                return;
            }

            for (var i = 0; i < huLength; i++) {
                self.showGameHuType(self.huedData[i]);
            };
        ...
        this.node.on('gang_notify',function(data){
            ...
            if (gangtype == "angang") {
                self.setplayEfxAndAudio(localIndex, "play_gang", cc.CGameConfigDataModel.getAudioURLByOther("angang",userId));
            }else {
                self.setplayEfxAndAudio(localIndex, "play_gang", cc.CGameConfigDataModel.getAudioURLByOther("gang",userId));
            }
        ...
        this.node.on("hangang_notify",function(data){
            ...
            self.setplayEfxAndAudio(localIndex, "play_gang",cc.CGameConfigDataModel.getAudioURLByOther("gang",data.userid));
        ...
        this.node.on("ting_notify",function(data){
            ...
            cc.vv.audioMgr.playMJGameSFX(cc.CGameConfigDataModel.getAudioURLByOther("ting",userId),true);
    ...
    mjClickedReturn: function (event, isStart) {
        ...
        var mjType = cc.vv.controlMgr.getMahjongType(target.mjId);
        if (mjType == 5) {
            console.log("it is huapai, it is not clicked.");
            return true;
        }

        // if (this.buhua_not_chupai == true) {
        //     console.log("it is playing huapai's times, holds is not clicked.");
        //     return true;
        // }
        ...
        if (this.judgeClickOtherTingMahjongs(target)) {
            console.log("target is others ting's mahjongs, target can not click.");
            return true; 
        }
    ...
    showAction:function(data){
        ...
        }else if(data && (data.hu || data.gang || data.peng || data.chi || data.canting || data.san)){  // 点弃后 判断能不能听
            this.send_action_tags = {
                hu: data.hu,
                peng: data.peng,
                gang: data.gang,
    ...
    onGameBeign:function() {
        ...
        cc.vv.gameNetMgr._gameOverDelayTime = 1;
        ...
        if (conf && conf.paoType) {
            var is_show_pao = true;
            if (conf.reset_count == 4) {
                is_show_pao = (cc.vv.gameNetMgr.numOfGames % 4 == 1)?true:false;
            }
    ...
    * 补花模块
    ...
    setHuaPaiList: function () {
        ...
        for(var i = 0; i < sides.length; ++i){
            ...
            this._huaPaiList.push(sideChild.getChildByName("huaPaiList"));
    ...
    getMyselfHuapai: function (userid) {
        ...
        for (var i = holds.length-1; i >= 0; i--) {
            ...
            if (cc.vv.gameNetMgr._gang_buhua == true) {
                if (i == holds.length-1 && mjType == 5) {
                    var huaMJid = holds[i];
                    seatData.holds.splice(i, 1);
                    localHuapai.push(huaMJid);
                }else {
                    cc.vv.gameNetMgr._gang_buhua = false;
                }
                break;
    ...
    sendHua: function (userid) {
        if (cc.vv.gameNetMgr._peng_chi == true || cc.vv.replayMgr.isReplay()) {
            cc.vv.gameNetMgr._peng_chi = false;
            return false;
        };
    ...
    showHuaPai: function (userid, data) {
        ...
        var localHuaCount = this._huaPaiList[localIndex].childrenCount;

        for (var i = 0; i < seatHuapaiLength; i++) {
            var index = i;
            if (localIndex == 1) {
                index = (seatHuapaiLength-1)-i;
            }
            var mjid = huapaiList[index];
            if (i < localHuaCount) {
                var sprite = this._huaPaiList[localIndex].children[i].getComponent("cc.Sprite");
                this.setSpriteFrameByMJID(pre,sprite,mjid);
            }else {
                var addHuaNode = cc.instantiate(this._huaPaiList[localIndex].children[0]);
                var sprite = addHuaNode.getComponent("cc.Sprite");
                this.setSpriteFrameByMJID(pre,sprite,mjid);
                this._huaPaiList[localIndex].addChild(addHuaNode);

            }
        };

        if (seatHuapaiLength > 0 && this._huaPaiList[localIndex].active == false) {
            this._huaPaiList[localIndex].active = true;
        }
    },
    ...
    hideAllHuaPai: function () {
        ...
        for (var i = 0; i < this._huaPaiList.length; i++) {
            var huaCount = this._huaPaiList[i].childrenCount;
            if (huaCount > 1) {
                for (var j = 1; j < huaCount; j++) {
                    this._huaPaiList[i].children[j].destroy();
                };
            }
            this._huaPaiList[i].active = false;
    ...
    initOtherMahjongs:function(seatData, setupHoldsCount, isSetup){
        ...
        if (cc.vv.replayMgr.isReplay() || seatData.isming) {
           bsNum =  holds.length;
        }
    ...
    setOtherReplayHolds: function (holds, num, bsNum, side, pre) {
        ...
        var localHoldsCount = holds_length + num + bsNum;
        if (localHoldsCount < 13) {
            if (cc.vv.alert) {
                cc.vv.alert.show("其他人的手牌有误！");
            }
            
            var distanceCount = 13 - localHoldsCount;
            for (var i = 0; i < distanceCount; i++) {
                var lostIdx = this.getMJIndex(side,localHoldsCount);
                var sprite = sideHolds.children[lostIdx].getComponent(cc.Sprite);
                sprite.node.active = false;
                sprite.node.removeAllChildren();
    ...
    initMahjongs:function(){
        ...
        var allOtherTingMahjongs = this.getAllOtherTingMJs();
        ...
        for(var i = 0; i < hold_length; ++i){
            ...
            this.showEqualOtherTingMJHoldsMask(sprite.node, allOtherTingMahjongs, isHoldMing);
        }
    ...
    *******************************************/
    ...
    setFlopData: function (data) {
        ...
        cc.vv.gameNetMgr._gameOverDelayTime += ((flopCount+1)*1000 + 500);
    ...
    *******************************************/
    ...
    setplayEfxAndAudio: function (localIndex, animationName, audioPath) {
        ...
        if (cc.vv.gameNetMgr.projectState != 'hideState' && audioPath != "") {
            cc.vv.audioMgr.playMJGameSFX(audioPath,true);
        }
    },
    ...
    chiOperation: function (data) {
        ...
        this.setplayEfxAndAudio(localIndex, "play_chi", cc.CGameConfigDataModel.getAudioURLByOther("chi",seatData.userid));
    ...
    pengOperation: function (data) {
        ...
        var localIndex = this.getLocalIndex(seatData.seatindex);
        this.setplayEfxAndAudio(localIndex, "play_peng", cc.CGameConfigDataModel.getAudioURLByOther("peng",userId));
    ...
    *******************************************/
    ...
    showGameHuType: function (data) {
        ...
        if (cc.vv.gameNetMgr._gameOverDelayTime < 2000) {
            cc.vv.gameNetMgr._gameOverDelayTime = 2000;
        }

        var hu_type = data.GameType, isGangChi = data.isgangkai, isQiangGang = data.isqianggang;
        this.showKDJPlayHu(localIndex, hu_type, isGangChi, isQiangGang);
    },
    ...
    showKDJPlayHu: function (localIndex, huType, gangchi, qianggang) {

        this.setHupaiSpr(localIndex, huType);

        if(huType == 1){
            this.setplayEfxAndAudio(localIndex, "play_zimo", cc.CGameConfigDataModel.getAudioURLByOther("zimo"));
        }else if (huType == 2) {
            this.setplayEfxAndAudio(localIndex, "play_hu", cc.CGameConfigDataModel.getAudioURLByOther("hu"));
        }
    },
    ...
    /*******************************************
    *** MODEL BEGIN (注释状态) ***
    *** 自己手牌中拥有其他玩家报听的牌，其牌不能打 ***
    *******************************************/

    judgeClickOtherTingMahjongs: function (target) {
    ...
    getAllOtherTingMJs: function () {
        /*
        var gameNetMgr = cc.vv.gameNetMgr;

        var allOtherTingMahjongs = [];
        for (var i = 0; i < gameNetMgr.seats.length; i++) {
            if (i != gameNetMgr.seatIndex) {
                var otherSeat = gameNetMgr.seats[i];
                if (otherSeat.seatHuPaiTips != null) {
                    for (var j = 0; j < otherSeat.seatHuPaiTips.length; j++) {
                        var otherTingMJ = otherSeat.seatHuPaiTips[j];

                        var isContinue = false;
                        var allOtherTingMJLength = allOtherTingMahjongs.length;
                        for (var k = 0; k < allOtherTingMJLength; k++) {
                            var getOtherTingMJ = allOtherTingMahjongs[k];
                            if (otherTingMJ == getOtherTingMJ) {
                                isContinue = true;
                                break;
                            }
                        };
                        ...
                        if (isContinue == false) {
                            allOtherTingMahjongs.push(otherTingMJ);
                        }
                    };
                }
            }
        ...
        return allOtherTingMahjongs;
        */
       return [];
    },

    judgeAllMask: function () {
        /*
        var isAllEqualOtherTingMJs = true;
        for (var i = 0; i < this._myMJArr.length-1; i++) {
            var sprite = this._myMJArr[i];
            var button = sprite.node.getComponent("cc.Button");
            if (sprite.node.active == true && button.interactable == true) {
                isAllEqualOtherTingMJs = false;
                break;
            }
        };

        return isAllEqualOtherTingMJs;
        */
        return false;
    },

    showMoPaiMask: function (moMJNode) {
        /*
        var mpButton = moMJNode.getComponent("cc.Button");
        mpButton.enableAutoGrayEffect = true;
        mpButton.interactable = true;

        var gameNetMgr = cc.vv.gameNetMgr;
        var mySeatData = gameNetMgr.seats[gameNetMgr.seatIndex];
        if (mySeatData.isming == true) {
            return;
        }

        var isAllEqualOtherTingMJs = this.judgeAllMask();

        if (isAllEqualOtherTingMJs == false) {
            var allOtherTingMahjongs = this.getAllOtherTingMJs();
            this.showEqualOtherTingMJHoldsMask(moMJNode, allOtherTingMahjongs, false);
        }
        */
    },

    showEqualOtherTingMJHoldsMask: function (holdMJNode, allOtherTingMahjongs, isming) {
        /*
        var button = holdMJNode.getComponent("cc.Button");
        ...
        var mjid = holdMJNode.mjId;

        var otherTingMJLength = allOtherTingMahjongs.length;
        for (var j = 0; j < otherTingMJLength; j++) {
            var otherTingMJid = allOtherTingMahjongs[j];
            if (mjid == otherTingMJid) {
                button.interactable = false;
                break;
            }else if (j == otherTingMJLength-1) {
                button.interactable = true;
            }
        };
        */
    ...
    initHuedData: function () {
        if (this.huedData == null) {
            this.huedData = [];
            return;
        }

        var hu_length = this.huedData.length;
        if (hu_length == 0) {
            this.huedData.splice(0, this.huedData.length);
        }
        this.huedData = [];
    },
