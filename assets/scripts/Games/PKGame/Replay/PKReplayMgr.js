...
cc.Class({
    ...
    init:function(data){
        ...
        console.log("meng pkrelayMgr data.action_records",this._actionRecords);
    ...
    takeAction:function(){
        ...
        if(action.type == ACTION_CHUPAI){
            ...
            cc.vv.gameNetMgr.doPKChupai(action.si,action.pai);
            return 1.0;
        }
            ...
            cc.vv.gameNetMgr.doPKjiaofen({seatindex:action.si.userId,difen:action.pai,seatIndex:action.si.seatIndex,cgType:16});
            return 1.0;
        }
        else if(action.type == ACTION_LAOLONG){
            // cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false, cgType: 10});
            cc.vv.gameNetMgr.doPKbujiao({seatindex:action.si.seatIndex})
            return 1.0;
        }
        else if(action.type == ACTION_LAOLAOLONG){
            console.log('242 laolaolong');
            // cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false, cgType: 15});
            cc.vv.gameNetMgr.doPKdizhu({seatindex:action.si.seatIndex,dipai:action.pai,userid:action.si.userId,cgType:15});
            return 1.0;
        }
        else if(action.type == ACTION_KAN){
            // cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false, cgType: 1});
            cc.vv.gameNetMgr.doTI({userid:action.si.userId});
            return 1.0;
        }
        else if(action.type == ACTION_LOUZI){
            // cc.vv.gameNetMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:true, cgType: 2});
            cc.vv.gameNetMgr.doBUTI({userid:action.si.userId});
            return 1.0;
        }else if(action.type==ACTION_FANTI){
            cc.vv.gameNetMgr.doFANTI({userid:action.si.userId});
            return 1.0;
        }
