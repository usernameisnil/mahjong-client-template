cc.Class({
    ...
    initBottomCards: function (cards) {
        ...
        for (let i = 0; i < this._bottomCards.length; ++i) {
            ...
            this.runCardsAction(card, cc.p((this._bottomCards.length - 1) * -0.5 * width * 0.4 + width * 0.4 * i, 310));
...
//
    ...
    onGameBeign: function () {
        ...
        var seatData = seats[cc.vv.ddzNetMgr.seatIndex];
        this.initCardTag();
        ...
        if (cc.vv.ddzNetMgr.isSync) {
            cc.find('Canvas/infobar/times').getComponent(cc.Label).string = cc.vv.ddzNetMgr.beishu;
        } else {
    ...
    playAni: function (cardList) {
        ...
        if (ddz.isPlane(cardList) || ddz.isPlaneWithOne(cardList) || ddz.isPlaneWithTwo(cardList)) {
            cc.vv.ddzNetMgr._gameOverDelayTime = 2000;
            this.play_feiji();
        }
        if (ddz.isKingBoom(cardList)) {
            cc.vv.ddzNetMgr._gameOverDelayTime = 2500;
            this.play_roket();
        }
        if (ddz.isDoubleScroll(cardList)) {
            cc.vv.ddzNetMgr._gameOverDelayTime = 500;
            this.play_liandui();
        }
        if (ddz.isScroll(cardList)) {
            cc.vv.ddzNetMgr._gameOverDelayTime = 1500;
            this.play_shunzi();
        }
        if (ddz.isFourBoom(cardList)) {
            cc.vv.ddzNetMgr._gameOverDelayTime = 700;
