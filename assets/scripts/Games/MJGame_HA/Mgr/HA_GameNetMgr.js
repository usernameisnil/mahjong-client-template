cc.Class({
    ...
    init: function () {
        ...
        this.mahjongPlayArray = ["chi", "peng", "gang", "hu", "ting", "ming", "tui", "san"];
    ...
    reset:function(){
        ...
        this._gang_buhua = false;
        this._peng_chi = false;
        ...
        if (this.seats instanceof Array) 
        {
            for(var i = 0; i < this.seats.length; ++i){
                ...
                this.seats[i].ispeng = (this.seats[i].ispeng == null)?null:false;
                this.seats[i].ismopai = (this.seats[i].ismopai == null)?null:false;
            }
        }
    },
    ...
    prepareReplay:function(roomInfo,detailOfGame){
        ...
        cc.vv.SelectRoom.setGameName(baseInfo.conf.game);
    ...
    addAllHunPai: function (passId) {
        ...
        var hun_rule = 1;//this.conf.hunCount;
    ...
    initHandlers:function(){
        ...
        cc.vv.net.addHandler("login_finished",function(data){ //add baihua2001cn cfg
        ...
        cc.vv.net.addHandler("exit_result",function(data){
            ...
            self.seats = null;
            self._gang_buhua = false;
            self._peng_chi = false;
            self.curaction = null;
        ...
        cc.vv.net.addHandler("dispress_push",function(data){
            ...
            console.log("wujun", "dispress_push");
            console.log(data);
        ...
        cc.vv.net.addHandler("game_sync_push",function(data){
            ...
            for (var i = 0; i < self.seats.length; ++i) {
                ...
                seat.ispeng = sd.ispeng || false;
                seat.ismopai = sd.ismopai || false;

                if(i == self.seatIndex){
                    self.dingque = sd.que;
                    self._gang_buhua = sd.ganging;
                }
           }
           self.isSync = true;
        ...
        cc.vv.net.addHandler("ming_notify_push",function(data){
            ...
            if (self.seats[seatIndex].seatHuPaiTips != null) {
                self.seats[seatIndex].seatHuPaiTips.splice(0, self.seats[seatIndex].seatHuPaiTips.length);
            }
            self.seats[seatIndex].seatHuPaiTips = data.seatHuPaiTips;
        ...
        cc.vv.net.addHandler("gang_notify_push",function(data){
            ...
            if (si == self.seatIndex) {
                self._gang_buhua = true;
            }
            
            self.doGang(si,pai,data.gangtype, data.turnindex, data.isgsg);
        });
        ...
        cc.vv.net.addHandler("game_hu_over_push",function(data){
            console.log("game_hu_over_push"); 
            console.log(data);

            self.dispatchEvent("game_huOver",data);
        });
    },
    ...
    doGang:function(seatIndex,pai,gangtype, turnindex, isgsg){
        var seatData = this.seats[seatIndex];
        ...
        this.dispatchEvent('gang_notify',{seatData:seatData,gangtype:gangtype, isgsg:isgsg});
    ...
    connectGameServer:function(data){   //add baihua2001cn cfg  用统一NET连接，如有特殊，可更改
        this.dissoveData = null;
    ...
    doReplyPlayOption: function (seatIndex, type, mjid) {
        ...
        // cc.log("wujun doReplyPlayOption optionData = ", optionData);

        var showReplyOptionData = this.sureReplyOptionData(optionData);
        // cc.log("wujun doReplyPlayOption showReplyOptionData = ", showReplyOptionData);

        this.dispatchEvent('game_replyPlayOption',showReplyOptionData);
